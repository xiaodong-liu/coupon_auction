...
def main(values, mus, sigmas, x_item, user_info, ad_idx):
    cvr_model = CVR_Model()
    ...
    payment_input = (rank_scores[torch.arange(100000), second_index] / cvrs[torch.arange(100000), winner_index]) + \
                    best_coupons[torch.arange(100000), winner_index]
    ...
    welfare = torch.mean(welfare_all[torch.arange(100000), winner_index] * allocation * is_ad)
    revenue = torch.mean(cvrs[torch.arange(100000), winner_index] * (
                payments - best_coupons[torch.arange(100000), winner_index]) * allocation * is_ad)
...
if __name__ == '__main__':
    ...
    i=40
    main(values, mus, sigmas, x_item, user_info, i)
