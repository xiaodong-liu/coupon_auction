...
def myerson_auction(ctr, valuation, ad_idx):
    ...
    # best_ctrs[:, ad_idx] = ctr[:, ad_idx]
    # best_coupon[:, ad_idx] = 0
    ...
    inverse_input = rank_score[np.arange(n_auction), second_index] / best_ctrs[np.arange(n_auction), winner_index] + \
                    best_coupon[np.arange(n_auction), winner_index]
    ...
    revenue = best_ctrs[np.arange(n_auction), winner_index] * (
                payment - best_coupon[np.arange(n_auction), winner_index]) * is_allocate
...
def main():
    ...
    social, revenue = myerson_auction(ctr, value, 2)
