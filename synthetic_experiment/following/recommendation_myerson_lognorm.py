...
def virtual_value_function(valuations):
    return valuations - (
                (1 - lognorm.cdf(valuations, s=1, scale=np.exp(3))) / lognorm.pdf(valuations, s=1, scale=np.exp(3)))
...
def myerson_auction(ctr, valuation, coupons, ad_idx):
    n_auction, n_bidder = valuation.shape
    ...
    inverse_input = rank_score[np.arange(n_auction), second_index] / ctr[np.arange(n_auction), winner_index] + coupons[
        np.arange(n_auction), winner_index]
    ...
    revenue = ctr[np.arange(n_auction), winner_index] * (
                payment - coupons[np.arange(n_auction), winner_index]) * is_allocate
    return np.mean(welfare[np.arange(n_auction), winner_index] * is_allocate * is_ad), np.mean(revenue * is_ad)
...
def main():
    ...
    ad_idx = 2
    # ctr_new[:, ad_idx] = ctr[:, ad_idx]
    # coupons[:, ad_idx] = 0
    social, revenue = myerson_auction(ctr_new, value, coupons, ad_idx)
