...
def second_price_auction(ctr, valuation, ad_idx):
    ...
    is_ad_idx = (rank_index[:, -1] == ad_idx)
    welfare = np.mean(social_welfare * is_ad_idx)
    pay = np.mean(payment * is_ad_idx)
    return welfare, pay
...
def main():
    ...
    ctr_new[:, 2] = ctr[:, 2]
    coupons[:, 2] = 0
    social, revenue = second_price_auction(ctr_new, valuation-coupons, 2)
